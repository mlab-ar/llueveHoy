...
function generateFakeForecast(location) {
  location = location || '-34.6131516,-58.3772316';
...
function getForecast(req, resp) {
  const location = req.params.location || '-34.6131516,-58.3772316';
  const url = `${BASE_URL}/${API_KEY}/${location}?lang=${LANG}&units=${UNIT}`;
  fetch(url).then((resp) => {
    if (resp.status !== 200) {
